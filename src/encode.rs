//! Range encoding and decoding functions for ML-DSA
//!
//! This module implements the BitPack and BitUnPack algorithms (Algorithm 17 in FIPS 204),
//! which encode polynomial coefficients in a range `[-a, b]` into compact byte arrays.
//!
//! The key idea is that a value `w` in `[-a, b]` can be mapped to `b - w` in `[0, a + b]`,
//! which can then be encoded using the simple bit-packing from `module_lattice::encode`.
//!
//! ## Range Encoding
//!
//! Given a pair `(A, B)` representing the range `[-A, B]`:
//! - The number of bits needed per coefficient is `ceil(log2(A + B + 1))`
//! - This is computed at the type level using `Length<Sum<A, B>>`
//!
//! ## Usage in ML-DSA
//!
//! - **Secret key polynomials** (`s1`, `s2`): range `[-eta, eta]`, encoded with `BitPack<Eta, Eta>`
//! - **Low-order parts** (`t0`): range `[-(2^d - 1), 2^d]`, encoded with `BitPack<2^d-1, 2^d>`
//! - **Signature polynomials** (`z`): range `[-(gamma1-1), gamma1]`, encoded with
//!   `BitPack<Gamma1-1, Gamma1>`
//! - **High-order parts** (`w1`): range `[0, b]`, encoded with simple `Encode`
...
/// Describes the encoding parameters for a range `[-A, B]`.
///
/// Given unsigned type-level integers `A` and `B`, this trait computes:
/// - `Min` = A (the magnitude of the negative bound)
/// - `Max` = B (the positive bound)
/// - `EncodingSize` = number of bits needed to encode values in `[0, A + B]`
///
/// The encoding bit width is `ceil(log2(A + B + 1))`, computed via `Length<Sum<A, B>>`.
pub trait RangeEncodingSize {
    /// The magnitude of the negative bound (A in [-A, B])
    type Min: Unsigned;
    /// The positive bound (B in [-A, B])
    type Max: Unsigned;
    /// The number of bits needed per coefficient
...
/// The negative bound magnitude for a range encoding pair `(A, B)`
pub(crate) type RangeMin<A, B> = <(A, B) as RangeEncodingSize>::Min;
/// The positive bound for a range encoding pair `(A, B)`
pub(crate) type RangeMax<A, B> = <(A, B) as RangeEncodingSize>::Max;
/// The number of encoding bits for a range encoding pair `(A, B)`
pub(crate) type RangeEncodingBits<A, B> = <(A, B) as RangeEncodingSize>::EncodingSize;
/// The byte size of an encoded polynomial for range `[-A, B]`
pub(crate) type RangeEncodedPolynomialSize<A, B> =
    <RangeEncodingBits<A, B> as EncodingSize>::EncodedPolynomialSize;
/// An encoded polynomial for range `[-A, B]`
pub(crate) type RangeEncodedPolynomial<A, B> = Array<u8, RangeEncodedPolynomialSize<A, B>>;
/// The byte size of an encoded vector for range `[-A, B]` with `K` polynomials
pub(crate) type RangeEncodedVectorSize<A, B, K> =
    <RangeEncodingBits<A, B> as VectorEncodingSize<K>>::EncodedVectorSize;
/// An encoded vector for range `[-A, B]` with `K` polynomials
pub(crate) type RangeEncodedVector<A, B, K> = Array<u8, RangeEncodedVectorSize<A, B, K>>;

/// Range-aware bit packing for polynomials and vectors.
///
/// Implements Algorithm 17 (BitPack / BitUnPack) from FIPS 204.
///
/// For a polynomial with coefficients in `[-A, B]`:
/// - **Pack**: maps each coefficient `w` to `b - w` (shifting to `[0, A+B]`), then encodes
/// - **Unpack**: decodes to `[0, A+B]`, then maps each value `z` back to `b - z`
pub(crate) trait BitPack<A, B> {
    /// The byte size of the packed representation
    type PackedSize: ArraySize;
    /// Pack (encode) the value into a compact byte array
    fn pack(&self) -> Array<u8, Self::PackedSize>;
    /// Unpack (decode) a byte array back into the original value
...
pub(crate) mod tests {
    ...
    use crate::module_lattice::encode::{EncodedPolynomial, EncodingSize};
    use core::ops::Rem;
